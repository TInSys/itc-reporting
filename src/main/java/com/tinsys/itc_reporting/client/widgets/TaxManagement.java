...
public class TaxManagement extends Composite implements WidgetSwitchManagement {
  ...
  final Column<TaxDTO, String> taxRateColumn = new Column<TaxDTO, String>(new ClickableTextCell()) {
  ...
  final Column<TaxDTO, String> taxPeriodColumn = new Column<TaxDTO, String>(new ClickableTextCell()) {
  ...
  public TaxManagement() {
    ...
    taxPeriodStartDateDateBox.setFormat(new DateBox.DefaultFormat(DateTimeFormat.getFormat(PredefinedFormat.DATE_SHORT)));
    taxPeriodStopDateDateBox.setFormat(new DateBox.DefaultFormat(DateTimeFormat.getFormat(PredefinedFormat.DATE_SHORT)));
  ...
  private void getTaxList() {
    taxService.getAllTaxs(currentZone, new AsyncCallback<ArrayList<TaxDTO>>() {
      ...
      @Override
      public void onFailure(Throwable caught) {
        Window.alert("Error fetching tax list :  " + caught.getMessage());
      }
    });
  }
  ...
  private Handler getSelectionChangeHandler() {
    return new SelectionChangeEvent.Handler() {
      ...
      public void onSelectionChange(SelectionChangeEvent event) {
        if ((editionInProgress && selectionModel.getSelectedObject() != selectedTax) || reverseSelection) {
          selectionModel.setSelected(selectedTax, true);
          taxRateTextBox.setText(oldTaxRateTextBoxContent);
          taxPeriodStartDateDateBox.setValue(oldTaxPeriodStartDateLabelContent);
          taxPeriodStopDateDateBox.setValue(oldTaxPeriodStopDateLabelContent);
          if (!reverseSelection) {
    ...
    if (taxIsValid()) {
      if (selectedTax == null) {
        ...
        taxService.updateTax(selectedTax, new AsyncCallback<TaxDTO>() {
          ...
          @Override
          public void onFailure(Throwable caught) {
            Window.alert("Error updating tax :" + caught.getMessage());
          }
  ...
  private void createTax() {
    taxService.createTax(selectedTax, periodToClose, new AsyncCallback<TaxDTO>() {
      ...
      @Override
      public void onFailure(Throwable caught) {
        Window.alert("Error saving new tax :" + caught.getMessage());
      }
    });
  }
  ...
  private boolean taxIsValid() {
    ...
    if (taxRateTextBox.getText().length() == 0 || !taxRateTextBox.getText().matches("\\-?[0-9]*\\.?[0-9]*")) {
    ...
    if (taxPeriodStopDateDateBox.getValue() != null && taxPeriodStartDateDateBox.getValue().getTime() > taxPeriodStopDateDateBox.getValue().getTime()) {
    ...
    if (errors.size() == 0) {
      ...
      if (selectedTax != null) {
        long newStartDate = taxPeriodStartDateDateBox.getValue().getTime();
        long newStopDate = (taxPeriodStopDateDateBox.getValue() != null) ? taxPeriodStopDateDateBox.getValue().getTime() : Long.MAX_VALUE;
          ...
          if (taxPosition == 0) {
            ...
            for (TaxDTO taxDTO : taxList) {
              ...
              if (taxDTO.getPeriod().getStopDate().getTime() >= newStopDate) {
                existingPostPeriodTaxToCheck = taxDTO.getPeriod().getStartDate().getTime();
                break;
              }
            }
        ...
        for (TaxDTO taxDTO : taxList) {
          ...
          long newStartDate = taxPeriodStartDateDateBox.getValue().getTime();
          long newStopDate = (taxPeriodStopDateDateBox.getValue() != null) ? taxPeriodStopDateDateBox.getValue().getTime() : Long.MAX_VALUE;
          long existingStartDate = taxDTO.getPeriod().getStartDate().getTime();
          long existingStopDate = (taxDTO.getPeriod().getStopDate() != null) ? taxDTO.getPeriod().getStopDate().getTime() : Long.MAX_VALUE;
          if (taxDTO != selectedTax) {
            if (existingStartDate > newStopDate && existingStopDate > newStopDate && currentIndex == 0) {
              errors.remove(periodError);
              break;
            }
            if (currentIndex == lastDateindex && newStartDate > existingStartDate && (newStartDate > existingStopDate || existingStopDate == Long.MAX_VALUE)) {
              errors.remove(periodError);
            ...
            if ((newStartDate <= existingStartDate && newStopDate >= existingStartDate) || (newStartDate <= existingStopDate && newStopDate >= existingStopDate)
                || (newStartDate <= existingStartDate && newStopDate >= existingStopDate)) {
              break;
            }

            if (existingStartDate < newStartDate && existingStopDate < newStartDate) {
