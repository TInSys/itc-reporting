...
public class ZoneManagement extends Composite implements WidgetSwitchManagement {
  ...
  final Column<ZoneDTO, String> zoneCodeColumn = new Column<ZoneDTO, String>(new ClickableTextCell()) {
  ...
  final Column<ZoneDTO, String> zoneNameColumn = new Column<ZoneDTO, String>(new ClickableTextCell()) {
  ...
  final Column<ZoneDTO, String> zoneCurrencyISOColumn = new Column<ZoneDTO, String>(new ClickableTextCell()) {
  ...
  public ZoneManagement() {
    ...
    zoneCodeTextBox.addValueChangeHandler(new ValueChangeHandler<String>() {
      public void onValueChange(ValueChangeEvent<String> arg0) {
        editionInProgress = true;
        cancelUpdateZone.setVisible(true);
      }
    });
    zoneNameTextBox.addValueChangeHandler(new ValueChangeHandler<String>() {
      public void onValueChange(ValueChangeEvent<String> arg0) {
        editionInProgress = true;
        cancelUpdateZone.setVisible(true);
      }
    });
    zoneCurrencyISOTextBox.addValueChangeHandler(new ValueChangeHandler<String>() {
      public void onValueChange(ValueChangeEvent<String> arg0) {
        editionInProgress = true;
        cancelUpdateZone.setVisible(true);
      }
  ...
  private Handler getSelectionChangeHandler() {
    return new SelectionChangeEvent.Handler() {
      ...
      public void onSelectionChange(SelectionChangeEvent event) {
        if ((editionInProgress && selectionModel.getSelectedObject() != selectedZone) || reverseSelection) {
          ...
          reverseSelection = false;
          selectedZone = selectionModel.getSelectedObject();
          if (selectedZone != null) {
            zoneCodeTextBox.setText(selectedZone.getCode());

            zoneNameTextBox.setText(selectedZone.getName());
  ...
  void handleClickSave(ClickEvent e) {
    if (zoneCodeTextBox.getText().length() > 0 && zoneNameTextBox.getText().length() > 0 && zoneCurrencyISOTextBox.getText().length() > 0) {
      if (selectedZone == null) {
        ...
        zoneService.createZone(selectedZone, new AsyncCallback<ZoneDTO>() {
          ...
          @Override
          public void onFailure(Throwable caught) {
            Window.alert("Error saving new zone :" + caught.getMessage());
          }
        });
      } else {
        selectedZone.setCode(zoneCodeTextBox.getText());
        ...
        zoneService.updateZone(selectedZone, new AsyncCallback<ZoneDTO>() {
          ...
          @Override
          public void onFailure(Throwable caught) {
            Window.alert("Error updating zone :" + caught.getMessage());
          }
      ...
      if (zoneCodeTextBox.getText().length() == 0) {
        errors.add("Zone code must not be empty");
      }
      if (zoneNameTextBox.getText().length() == 0) {
        errors.add("Zone name must not be empty");
      }
      if (zoneCurrencyISOTextBox.getText().length() == 0) {
        errors.add("Zone currency must not be empty");
      }
      showSaveAlert(errors);
    }
  }
