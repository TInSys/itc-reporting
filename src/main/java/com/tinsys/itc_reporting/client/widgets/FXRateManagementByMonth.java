...
public class FXRateManagementByMonth extends Composite implements WidgetSwitchManagement {
  ...
  final Column<FXRateDTO, String> fxRateRateColumn = new Column<FXRateDTO, String>(fxRateRateCell) {

    @Override
    public String getValue(FXRateDTO fxRateDTO) {
      return (fxRateDTO.getRate() != null) ? fxRateDTO.getRate().toString() : "0";
    }
  };
  CustomTextInputCell fxRateISOCurrencyCell = new CustomTextInputCell();
  final Column<FXRateDTO, String> fxRateISOCurrencyColumn = new Column<FXRateDTO, String>(fxRateISOCurrencyCell) {
  ...
  final Column<FXRateDTO, String> deleteFXRateColumn = new Column<FXRateDTO, String>(new ButtonCell()) {
  ...
  private void getFXRateList() {
    ...
    fxRateService.getAllFXRatesForPeriod(monthPeriodDto, new AsyncCallback<ArrayList<FXRateDTO>>() {
      ...
      public void onSuccess(ArrayList<FXRateDTO> result) {
        ...
        if (currentPage == 999) {
          currentPage = ((result.size() - 1) < 0) ? 0 : (result.size() - 1) / 15;
        }
  ...
  void handleClickSave(ClickEvent e) {
    if (errorList.size() == 0) {
      fxRateService.saveOrUpdate(this.fxRateList, new AsyncCallback<Void>() {
        ...
        @Override
        public void onFailure(Throwable caught) {
          Window.alert("Error saving/updating  fxRates :" + caught.getMessage());
        }
      ...
      for (FXRateDTO fxRateDTO : errorList) {
        dummyList.add("Rate " + fxRateDTO.getRate() + " at line " + (fxRateList.indexOf(fxRateDTO) + 1)
