...
public class ApplicationManagement extends Composite implements WidgetSwitchManagement {
  ...
  final Column<ApplicationDTO, String> applicationVendorIDColumn = new Column<ApplicationDTO, String>(new ClickableTextCell()) {
  ...
  final Column<ApplicationDTO, String> applicationNameColumn = new Column<ApplicationDTO, String>(new ClickableTextCell()) {
  ...
  public ApplicationManagement() {
    ...
    applicationVendorIDTextBox.addValueChangeHandler(new ValueChangeHandler<String>() {
      public void onValueChange(ValueChangeEvent<String> arg0) {
        editionInProgress = true;
        cancelUpdateApplication.setVisible(true);
      }
    });
    applicationNameTextBox.addValueChangeHandler(new ValueChangeHandler<String>() {
      public void onValueChange(ValueChangeEvent<String> arg0) {
        editionInProgress = true;
        cancelUpdateApplication.setVisible(true);
      }
  ...
  private Handler getSelectionChangeHandler() {
    return new SelectionChangeEvent.Handler() {
      ...
      public void onSelectionChange(SelectionChangeEvent event) {
        if ((editionInProgress && selectionModel.getSelectedObject() != selectedApplication) || reverseSelection) {
          ...
          reverseSelection = false;
          selectedApplication = selectionModel.getSelectedObject();
          if (selectedApplication != null) {
            applicationVendorIDTextBox.setText(selectedApplication.getVendorID());

            applicationNameTextBox.setText(selectedApplication.getName());
            cancelUpdateApplication.setVisible(true);
  ...
  void handleClickSave(ClickEvent e) {
    if (applicationVendorIDTextBox.getText().length() > 0 && applicationNameTextBox.getText().length() > 0) {
      if (selectedApplication == null) {
        ...
        applicationService.createApplication(selectedApplication, new AsyncCallback<ApplicationDTO>() {
          ...
          @Override
          public void onFailure(Throwable caught) {
            Window.alert("Error saving new application :" + caught.getMessage());
          }
        });
  ...
  private void deleteApplication() {
    applicationService.deleteApplication(selectedApplication, new AsyncCallback<Void>() {
      ...
      @Override
      public void onFailure(Throwable caught) {
        Window.alert("Error deleting application :" + caught.getMessage());
      }
