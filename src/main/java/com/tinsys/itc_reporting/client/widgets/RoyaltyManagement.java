...
public class RoyaltyManagement extends Composite implements WidgetSwitchManagement {
  ...
  final Column<RoyaltyDTO, String> royaltyApplicationColumn = new Column<RoyaltyDTO, String>(new ClickableTextCell()) {
  ...
  public RoyaltyManagement() {
      ...
      public void onSuccess(ArrayList<CompanyDTO> result) {
        ...
        zoneService.getAllZones(new AsyncCallback<ArrayList<ZoneDTO>>() {
          ...
          public void onSuccess(ArrayList<ZoneDTO> result) {
            ...
            allAvailableZones = (List<ZoneDTO>) result.clone();
    ...
    shareOnSalesRadioButton.addValueChangeHandler(new ValueChangeHandler<Boolean>() {
    ...
    shareOnProceedsRadioButton.addValueChangeHandler(new ValueChangeHandler<Boolean>() {
    ...
    royaltyCellTable.setEmptyTableWidget(new HTML("There is no data to display"));
    royaltyCellTable.setTableLayoutFixed(true);
    royaltyCellTable.addColumn(royaltyApplicationColumn, "Royalty for Application ");
    ...
    applicationListBox.addChangeHandler(new ChangeHandler() {
      ...
      @Override
      public void onChange(ChangeEvent event) {
        oldApplicationDTO = ((ListBox) event.getSource()).getSelectedIndex();
        editionInProgress = true;
      }
    });
  ...
  private Handler getSelectionChangeHandler() {
    return new SelectionChangeEvent.Handler() {
      ...
      public void onSelectionChange(SelectionChangeEvent event) {
        if ((editionInProgress && selectionModel.getSelectedObject() != selectedRoyalty) || reverseSelection) {
          selectionModel.setSelected(selectedRoyalty, true);
          shareRateTextBox.setText(oldShareRateTextBoxContent);
          shareOnSalesRadioButton.setValue(oldShareRateCalculationField.equals(CALCULATION_ON_SALES));
          shareOnProceedsRadioButton.setValue(oldShareRateCalculationField.equals(CALCULATION_ON_PROCEEDS));
          shuttleBox.getAvailableItemsListbox().clear();
          for (ZoneDTO zoneDTO : oldAvailableZones) {
            shuttleBox.getAvailableItemsListbox().addItem(zoneDTO.getName(), String.valueOf(zoneDTO.getId()));
          }
          shuttleBox.getSelectedItemsListbox().clear();
          for (ZoneDTO zoneDTO : oldSelectedZones) {
            shuttleBox.getSelectedItemsListbox().addItem(zoneDTO.getName(), String.valueOf(zoneDTO.getId()));
          }
          ...
          if (selectedRoyalty != null) {
            selectionModel.setSelected(selectedRoyalty, true);
            SortableListBox availableZonesListbox = new SortableListBox(true);
            SortableListBox selectedZonesListbox = new SortableListBox(true);
            ...
            for (ZoneDTO zoneDTO : availableZones) {
              availableZonesListbox.addItem(zoneDTO.getName(), String.valueOf(zoneDTO.getId()));
            }
            shuttleBox.setAvailableItemsListbox(availableZonesListbox);
            for (ZoneDTO zoneDTO : selectedRoyalty.getZones()) {
              selectedZonesListbox.addItem(zoneDTO.getName(), String.valueOf(zoneDTO.getId()));
            }
            shuttleBox.setSelectedItemsListbox(selectedZonesListbox);
            shuttleBox.refreshLayout();
            oldAvailableZones = getAvailableZones();
            oldSelectedZones = getSelectedZones();
            shareRateTextBox.setText(selectedRoyalty.getShareRate().toPlainString());
            shareOnSalesRadioButton.setValue(selectedRoyalty.getShareRateCalculationField().equals(CALCULATION_ON_SALES));
            shareOnProceedsRadioButton.setValue(selectedRoyalty.getShareRateCalculationField().equals(CALCULATION_ON_PROCEEDS));
    ...
    if (royaltyIsValid()) {
      if (selectedRoyalty == null) {
        ...
        for (ApplicationDTO applicationDTO : availableApplications) {
          if (applicationDTO.getId() == Integer.parseInt(applicationListBox.getValue(applicationListBox.getSelectedIndex()))) {
            selectedRoyalty.setApplication(applicationDTO);
            break;
          }
        }
      }
      selectedRoyalty.setShareRate(new BigDecimal(shareRateTextBox.getText()));
      selectedRoyalty.setShareRateCalculationField((shareOnSalesRadioButton.getValue()) ? CALCULATION_ON_SALES : CALCULATION_ON_PROCEEDS);
      List<ZoneDTO> tmpZones = getSelectedZones();
      selectedRoyalty.setZones(tmpZones);
      if (selectedRoyalty.getId() != null) {
        royaltyService.updateRoyalty(selectedRoyalty, new AsyncCallback<RoyaltyDTO>() {

          @Override
          public void onFailure(Throwable caught) {
            Window.alert("Error saving Royalty :  " + caught.getMessage());
          }
        ...
        royaltyService.createRoyalty(selectedRoyalty, new AsyncCallback<RoyaltyDTO>() {
  ...
  void handleAddRoyalty(ClickEvent e) {
    if (editionInProgress) {
      ...
      if (currentCompany != null && currentCompany != dummyCompany) {
        ...
        selectedRoyalty = null;
        availableApplications = (List<ApplicationDTO>) allAvailableApplications.clone();
        if (royaltyList != null) {
        ...
        applicationListBox.addItem(dummyApplication.getName(), String.valueOf(dummyApplication.getId()));
        for (ApplicationDTO applicationDTO : availableApplications) {
          applicationListBox.addItem(applicationDTO.getName(), String.valueOf(applicationDTO.getId()));
        }
        shuttleBox.reset();
        availableZones.clear();
        availableZones.addAll(allAvailableZones);
  ...
  private List<ZoneDTO> getAvailableZones() {
    ...
    for (int i = 0; i < shuttleBox.getAvailableItemsListbox().getItemCount(); i++) {
      for (ZoneDTO zoneDTO : allAvailableZones) {
        if (zoneDTO.getId() == Integer.parseInt(shuttleBox.getAvailableItemsListbox().getValue(i))) {
          tmpZone.add(zoneDTO);
          break;
        }
      }
  ...
  private List<ZoneDTO> getSelectedZones() {
    ...
    for (int i = 0; i < shuttleBox.getSelectedItemsListbox().getItemCount(); i++) {
      for (ZoneDTO zoneDTO : allAvailableZones) {
        if (zoneDTO.getId() == Integer.parseInt(shuttleBox.getSelectedItemsListbox().getValue(i))) {
          tmpZone.add(zoneDTO);
          break;
        }
      }
  ...
  private boolean royaltyIsValid() {
    ...
    if ((selectedRoyalty == null && applicationListBox.getSelectedIndex() <= 0) || (selectedRoyalty != null && selectedRoyalty.getApplication() == null)) {
      errors.add("Please select an application");
      valid = false;
    }
    if (!shareRateTextBox.getText().matches("\\-?[0-9]*\\.?[0-9]*") || shareRateTextBox.getText().length() == 0) {
