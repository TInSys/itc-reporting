...
public class FiscalPeriodDAOImpl implements FiscalPeriodDAO {
    ...
    public FiscalPeriodDTO updatePeriod(FiscalPeriodDTO aPeriod) {
        FiscalPeriod period = (FiscalPeriod) factory.getCurrentSession().get(FiscalPeriod.class,
                aPeriod.getId());
   ...
   public FiscalPeriod findPeriod(FiscalPeriod aPeriod) {
      FiscalPeriod period = (FiscalPeriod) factory.getCurrentSession().createCriteria(FiscalPeriod.class).add(Restrictions.eq("year", aPeriod.getYear())).add(Restrictions.eq("month", aPeriod.getMonth())).uniqueResult();
      return period;
   }
