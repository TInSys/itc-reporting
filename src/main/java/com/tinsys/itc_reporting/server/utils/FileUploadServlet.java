...
public class FileUploadServlet extends HttpServlet {
   ...
   protected void doPost(HttpServletRequest req, HttpServletResponse resp)
      ...
      if (ServletFileUpload.isMultipartContent(req)) {
         ...
         resp.getWriter()
               .write(
                     "ERROR : Request contents type is not supported by the servlet.");
         resp.flushBuffer();
      }
   }
   ...
   private void parseFile(FileItem item) throws IOException {
      ...
      String fileYear = item.getName().substring(periodIndex + 3,
            periodIndex + 5);
      if ((!fileMonth.matches("[0-9]+") || Integer.parseInt(fileMonth) < 1 || Integer
            .parseInt(fileMonth) > 12) || !fileYear.matches("[0-9]+")) {
         tmpErrorList.add("Period can't be parsed in the file name :"
               + item.getName());
         notProcessedFiles.add(item.getName());
         logger.error("Period can't be parsed in the file name :"
               + item.getName());
         errorList.addAll(tmpErrorList);
         return;
      }
      String fileName = item.getName()
            .substring(0, item.getName().indexOf('.'));
      ...
      if (!duplicateFileCheck.contains(fileName)) {
         ...
         if (zone == null) {
            tmpErrorList.add("No corresponding Zone found in database for :"
                  + code + " . File " + item.getName() + " won't be processed");
            notProcessedFiles.add(item.getName());
            errorList.addAll(tmpErrorList);
            return;
         }
         ...
         tmpErrorList.add("There's already a file with name " + fileName
               + " in this batch");
         notProcessedFiles.add(item.getName());
         logger.debug("There's already a file with name " + fileName
               + " in this batch");
         errorList.addAll(tmpErrorList);
      }
